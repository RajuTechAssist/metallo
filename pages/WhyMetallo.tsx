...
const convergingItems = [
  ...
  { deg: 51.4, icon: 'electrical_services', label: 'Wire & Cables' },
  { deg: 102.8, icon: 'horizontal_rule', label: 'Cable Tray' },
  { deg: 154.2, icon: 'whatshot', label: 'Welding' },
  { deg: 205.7, icon: 'construction', label: 'Power Tools' },
  { deg: 257.1, icon: 'water_drop', label: 'Pipes' },
  { deg: 308.5, icon: 'architecture', label: 'Structures' },
];
...
const WhyMetallo: React.FC = () => {
  ...
  return (
    <div className="w-full bg-white overflow-x-hidden">
      ...
      <section
        className="relative w-full overflow-hidden flex flex-col md:flex-row bg-slate-900"
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full flex flex-wrap md:flex-nowrap">
          {spectrumVerticals.map((item, index) => (
            <div
              key={item.id}
              className="relative w-1/2 h-1/4 sm:w-1/3 md:w-auto md:flex-1 md:h-full border-r border-b border-gray-800/50 group"
        ...
        {/* Left-Aligned Text Box matching Product Heroes */}
        <div className={`absolute inset-0 z-20 flex flex-col justify-center pointer-events-none ${CONTAINER}`}>
          <div className="max-w-2xl pointer-events-auto bg-slate-900/60 pt-10 pb-10 pl-8 pr-8 lg:pl-10 lg:pr-10 transition-all duration-300 transform">
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-heading font-extrabold text-white mb-2 leading-[1.12]">
              {hoveredIndex !== null ? spectrumVerticals[hoveredIndex].name.toUpperCase() : "MAXIMIZE EFFICIENCY."}
            </h1>
            <h2 className="text-lg md:text-2xl font-heading font-medium text-metallo-gold tracking-widest mb-6 uppercase">
              {hoveredIndex !== null ? spectrumVerticals[hoveredIndex].cert : "One Platform. Seven Verticals. Zero Friction."}
            </h2>
            <div className={`transition-opacity duration-300 ${hoveredIndex !== null ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100 h-auto'}`}>
        ...
        {/* Bottom accent stripe matching Product Heroes */}
        <div className="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-yellow-500 via-yellow-500/60 to-transparent z-30 pointer-events-none" />
      </section>
