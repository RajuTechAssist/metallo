                  ...
                  <motion.div key={activeProduct.Model} variants={DETAIL_VARIANTS} initial="initial" animate="animate" exit="exit">
                    <div className="mb-8">
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        ...
                        <span className="text-xs font-bold font-heading uppercase tracking-[0.15em] text-slate-500 bg-slate-100 px-3 py-1 rounded-sm">{activeProduct.Model}</span>
                      </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {activeProduct.Features.map((feat, idx) => (
                            <div key={idx} className="flex items-start gap-3 p-3 bg-slate-50 rounded-sm border border-slate-100">
                              ...
                              <span className="text-sm font-sans text-slate-700 leading-relaxed">{feat}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {activeProduct.Variants && activeProduct.Variants.length > 0 && (
                      <div className="mb-10">
                        <h3 className="text-xs font-heading font-bold uppercase tracking-[0.15em] text-slate-400 mb-5 flex items-center gap-2">
                          <span className="material-symbols-outlined text-sm text-yellow-500">inventory</span>
                          Available Variants
                        </h3>
                          ...
                          <table className="w-full text-sm font-sans">
                            <thead>
                              <tr className="border-b-2 border-slate-200">
                                <th className="text-left py-3 px-4 text-[11px] font-heading font-bold uppercase tracking-widest text-slate-400">Product Code</th>
                                <th className="text-left py-3 px-4 text-[11px] font-heading font-bold uppercase tracking-widest text-slate-400">Type</th>
                                <th className="text-left py-3 px-4 text-[11px] font-heading font-bold uppercase tracking-widest text-slate-400">Battery</th>
                                <th className="text-left py-3 px-4 text-[11px] font-heading font-bold uppercase tracking-widest text-slate-400">Storage</th>
                              </tr>
                            </thead>
                            <tbody>
                              {activeProduct.Variants.map((v) => (
                                <tr key={v.code} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                  <td className="py-3 px-4 font-mono text-xs font-bold text-slate-800">{v.code}</td>
                                  <td className="py-3 px-4 text-slate-600">{v.type}</td>
                                  <td className="py-3 px-4 text-slate-600">{v.battery || "—"}</td>
                                  <td className="py-3 px-4 text-slate-600">{v.storage || "—"}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {activeProduct.Accessories && activeProduct.Accessories.length > 0 && (
                      <div className="mb-10">
                        <h3 className="text-xs font-heading font-bold uppercase tracking-[0.15em] text-slate-400 mb-5 flex items-center gap-2">
                          <span className="material-symbols-outlined text-sm text-yellow-500">handyman</span>
                          Included Accessories
                        </h3>
                        <div className="flex flex-wrap gap-2">
                          {activeProduct.Accessories.map((acc, idx) => (
                            <span key={idx} className="inline-flex items-center gap-1.5 px-4 py-2 bg-yellow-50 border border-yellow-200 text-yellow-800 text-xs font-heading font-bold uppercase tracking-wider rounded-sm">
                              <span className="material-symbols-outlined text-sm">check_circle</span>{acc}
                            </span>
                          ))}
                        </div>
                      </div>
