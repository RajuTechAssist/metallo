...
const DieCasting: React.FC = () => {
  ...
  return (
    <div className="w-full bg-slate-50" style={{ overflowX: "clip" }}>
      <ProductHero backgroundImage="/diecasting/die-casting-process-foundry.jpg" title="Die Casting" subtitle="Precision Engineered Components." description="Aluminium & zinc die cast components with in-house tooling, CNC machining, and surface finishing — from prototype to mass production." breadcrumbLabel="Die Casting" ctaLabel="Download Brochure" />
                ...
                {activeProduct && (
                  <motion.div key={activeProduct["Product Name"]} variants={DETAIL_VARIANTS} initial="initial" animate="animate" exit="exit">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                      {[{ label: "Material", value: activeProduct.Material, icon: "category" }, { label: "Process", value: activeProduct.Process, icon: "precision_manufacturing" }, { label: "Tolerance", value: activeProduct.Tolerance, icon: "straighten" }].map((info) => (
                        <div key={info.label} className="bg-slate-900 text-white p-5 rounded-sm">
                          <div className="flex items-center gap-2 mb-2"><span className="material-symbols-outlined text-base text-yellow-500">{info.icon}</span><span className="text-[10px] font-heading font-bold uppercase tracking-widest text-slate-400">{info.label}</span></div>
                    ...
                    <div className="flex flex-wrap gap-3 mb-10">
                      <button className="inline-flex items-center gap-2 px-6 py-3 bg-yellow-500 text-slate-900 text-xs font-heading font-bold uppercase tracking-wider hover:bg-yellow-400 transition-colors shadow-sm"><span className="material-symbols-outlined text-lg">upload_file</span>Upload Drawing</button>
      ...
      <ProductQABanner title="Manufacturing Excellence" items={QA_ITEMS} />
      <ProductCTA title="Have a Die Casting Project?" description="Share your 3D CAD file for a feasibility study, die quotation, and per-piece pricing." ctaLabel="Upload Drawing" ctaIcon="upload_file" />
    </div>
  );
};
