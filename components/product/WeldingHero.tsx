"use client";

import React, { useState, useEffect, useCallback, useRef } from "react";
import { motion, AnimatePresence, type Variants } from "framer-motion";
import Link from "next/link";
import { CONTAINER } from "./productLayout";

/* ═══════════════════════════════════════════════════════════════
   WELDING HERO SLIDER
   Full-screen cinematic slider for the Welding & Allied page.
   3 slides showcasing WB Alloys integration, global footprint,
   and safety compliance.
   ═══════════════════════════════════════════════════════════════ */

const SLIDE_DURATION = 7; // seconds per slide

interface WeldingSlide {
  id: string;
  headline: string;
  subHeadline: string;
  body: string;
  cta: string;
  ctaLink: string;
  image: string;
}

const SLIDES: WeldingSlide[] = [
  {
    id: "01",
    headline: "Precision Welding, Engineered by Legacy.",
    subHeadline: "Metallo Welding, powered by WB Alloys.",
    body: "Combining an agile, borderless supply network with decades of specialized metallurgical engineering and manufacturing.",
    cta: "Explore the Ecosystem",
    ctaLink: "#ecosystem",
    image: "https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?auto=format&fit=crop&w=1920&q=80",
  },
  {
    id: "02",
    headline: "Transnational Metallurgical Intelligence.",
    subHeadline: "From Glasgow to Virginia, and the UAE.",
    body: "Deploying world-class welding engineers, NDT/NDE specialists, and high-integrity consumables directly to your global mega-projects.",
    cta: "View Site Services",
    ctaLink: "#site-services",
    image: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=1920&q=80",
  },
  {
    id: "03",
    headline: "Uncompromising Tier-1 Compliance.",
    subHeadline: "ISO 9001 | ISO 14001 | ISO 45001 Certified.",
    body: "Engineered for absolute safety. Featuring state-of-the-art fume extraction systems, PAPR respiratory protection, and on-site LEV certification.",
    cta: "Explore Safety & PPE",
    ctaLink: "?category=safety",
    image: "https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?auto=format&fit=crop&w=1920&q=80",
  },
];

/* ── Easing Curves ── */
const EASE_OUT: [number, number, number, number] = [0.16, 1, 0.3, 1];
const EASE_IN: [number, number, number, number] = [0.55, 0, 1, 0.45];
const EASE_STANDARD: [number, number, number, number] = [0.25, 0.1, 0.25, 1];

/* ── Animation Variants ── */

const subHeadlineVariants: Variants = {
  initial: { opacity: 0, x: -20 },
  animate: {
    opacity: 1,
    x: 0,
    transition: { duration: 0.5, ease: EASE_STANDARD },
  },
  exit: { opacity: 0, x: 20, transition: { duration: 0.3 } },
};

const headlineVariants: Variants = {
  initial: { y: "110%" },
  animate: {
    y: 0,
    transition: {
      duration: 0.8,
      ease: EASE_OUT,
      delay: 0.15,
    },
  },
  exit: {
    y: "-110%",
    transition: { duration: 0.5, ease: EASE_IN },
  },
};

const bodyVariants: Variants = {
  initial: { opacity: 0, y: 10 },
  animate: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, ease: "easeOut", delay: 0.55 },
  },
  exit: { opacity: 0, y: -10, transition: { duration: 0.3 } },
};

const ctaVariants: Variants = {
  initial: { opacity: 0, y: 15 },
  animate: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.5, ease: "easeOut", delay: 0.8 },
  },
  exit: { opacity: 0, transition: { duration: 0.2 } },
};

const breadcrumbVariants: Variants = {
  initial: { opacity: 0 },
  animate: {
    opacity: 1,
    transition: { duration: 0.4, delay: 0.1 },
  },
};

const TRANSITION_LOCK_MS = 800;

const WeldingHero: React.FC = () => {
  const [current, setCurrent] = useState(0);
  const [progressKey, setProgressKey] = useState(0);

  const currentRef = useRef(current);
  currentRef.current = current;

  const isTransitioning = useRef(false);
  const lockTimer = useRef<ReturnType<typeof setTimeout>>(undefined);

  const goToSlide = useCallback((index: number) => {
    if (isTransitioning.current) return;
    isTransitioning.current = true;
    clearTimeout(lockTimer.current);
    lockTimer.current = setTimeout(() => {
      isTransitioning.current = false;
    }, TRANSITION_LOCK_MS);

    setCurrent(index);
    setProgressKey((k) => k + 1);
  }, []);

  const next = useCallback(() => {
    goToSlide((currentRef.current + 1) % SLIDES.length);
  }, [goToSlide]);

  const prev = useCallback(() => {
    goToSlide((currentRef.current - 1 + SLIDES.length) % SLIDES.length);
  }, [goToSlide]);

  /* Auto-advance */
  useEffect(() => {
    const timer = setTimeout(() => {
      isTransitioning.current = false;
      next();
    }, SLIDE_DURATION * 1000);
    return () => clearTimeout(timer);
  }, [progressKey, next]);

  const slide = SLIDES[current];

  return (
    <section className="relative w-full bg-slate-900 overflow-hidden h-[50dvh] md:h-[calc(80vh-80px)] min-h-[400px] md:min-h-[500px]">
      {/* ── Background Images with Ken Burns ── */}
      <AnimatePresence mode="sync">
        <motion.div
          key={slide.id}
          className="absolute inset-0 z-0 w-full"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration: 1.2, ease: "easeInOut" }}
        >
          <motion.img
            src={slide.image}
            alt={slide.headline}
            className="w-full h-full object-cover"
            initial={{ scale: 1 }}
            animate={{ scale: 1.08 }}
            transition={{ duration: SLIDE_DURATION + 1, ease: "linear" }}
          />
        </motion.div>
      </AnimatePresence>

      {/* ── Overlays ── */}
      <div className="absolute inset-0 z-[1] bg-slate-900/60" />
      <div className="absolute inset-x-0 bottom-0 h-[40%] z-[2] bg-gradient-to-t from-slate-900/80 via-slate-900/30 to-transparent" />

      {/* ── Content ── */}
      <div className={`relative z-10 flex flex-col justify-center h-full pt-[5%] pb-[10%] ${CONTAINER}`}>
        <div className="max-w-5xl">
          {/* Breadcrumb (always visible) */}
          <motion.nav
            className="flex items-center gap-2 text-sm text-slate-400 mb-6 font-sans"
            variants={breadcrumbVariants}
            initial="initial"
            animate="animate"
          >
            <Link href="/" className="hover:text-white transition-colors">
              Home
            </Link>
            <span className="material-symbols-outlined text-xs">
              chevron_right
            </span>
            <Link href="/products/welding" className="hover:text-white transition-colors">
              Welding & Allied
            </Link>
          </motion.nav>


          <AnimatePresence mode="wait">
            <motion.div key={slide.id}>
              {/* Sub-Headline */}
              <motion.div
                className="flex items-center gap-[clamp(0.5rem,1.5%,0.75rem)] mb-[clamp(0.5rem,1.5%,1rem)]"
                variants={subHeadlineVariants}
                initial="initial"
                animate="animate"
                exit="exit"
              >
                <span className="block w-8 h-px bg-yellow-500" />
                <span className="text-xs font-bold font-heading text-yellow-500 uppercase tracking-[0.2em]">
                  {slide.id} &mdash; {slide.subHeadline}
                </span>
              </motion.div>

              {/* Headline (Masked Reveal) */}
              <div className="overflow-hidden mb-[clamp(0.75rem,2%,1.5rem)] pb-2">
                <motion.h1
                  className="text-[clamp(2rem,5vw,3.75rem)] font-heading font-extrabold text-white leading-[1.15] tracking-tight"
                  variants={headlineVariants}
                  initial="initial"
                  animate="animate"
                  exit="exit"
                >
                  {slide.headline}
                </motion.h1>
              </div>

              {/* Body */}
              <motion.p
                className="text-[clamp(0.875rem,1.2vw,1.125rem)] text-gray-300 font-sans max-w-xl leading-relaxed mb-[clamp(1.5rem,4%,2.5rem)]"
                variants={bodyVariants}
                initial="initial"
                animate="animate"
                exit="exit"
              >
                {slide.body}
              </motion.p>

              {/* CTA */}
              <motion.div
                variants={ctaVariants}
                initial="initial"
                animate="animate"
                exit="exit"
              >
                <Link
                  href={slide.ctaLink}
                  className="inline-flex items-center gap-[clamp(0.5rem,1%,0.75rem)] px-[clamp(1rem,2vw,1.5rem)] py-[clamp(0.5rem,1vw,0.75rem)] bg-yellow-500 text-slate-900 text-[clamp(0.75rem,0.9vw,0.875rem)] font-heading font-extrabold uppercase tracking-wider hover:bg-yellow-400 transition-colors"
                >
                  {slide.cta}
                  <span className="material-symbols-outlined text-lg">
                    arrow_forward
                  </span>
                </Link>
              </motion.div>
            </motion.div>
          </AnimatePresence>
        </div>
      </div>

      {/* ── Track Navigation ── */}
      <div className={`absolute flex justify-between items-center bottom-0 inset-x-0 z-20 pb-[3%] ${CONTAINER}`}>
        {/* Progress Dots */}
        <div className="flex items-center gap-3">
          {SLIDES.map((s, i) => (
            <button
              key={s.id}
              onClick={() => goToSlide(i)}
              className={`h-[2px] transition-all duration-300 ${i === current
                ? "w-10 bg-yellow-500"
                : "w-5 bg-white/25 hover:bg-white/50"
                }`}
              aria-label={`Go to slide ${s.id}`}
            />
          ))}
        </div>

        {/* Arrow Buttons */}
        <div className="flex items-center gap-2">
          <button
            onClick={prev}
            className="w-12 h-12 border border-white/20 text-white/70 hover:bg-white/10 hover:text-white hover:border-white/40 transition-all flex items-center justify-center"
            aria-label="Previous slide"
          >
            <span className="material-symbols-outlined text-xl">
              arrow_back
            </span>
          </button>
          <button
            onClick={next}
            className="w-12 h-12 border border-white/20 text-white/70 hover:bg-white/10 hover:text-white hover:border-white/40 transition-all flex items-center justify-center"
            aria-label="Next slide"
          >
            <span className="material-symbols-outlined text-xl">
              arrow_forward
            </span>
          </button>
        </div>
      </div>

      {/* Bottom accent stripe */}
      <div className="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-yellow-500 via-yellow-500/60 to-transparent z-30" />
    </section>
  );
};

export default WeldingHero;
